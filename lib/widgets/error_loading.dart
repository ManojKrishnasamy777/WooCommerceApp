...
class ErrorView extends HookWidget {
    ...
    return SafeArea(
      child: Center(
        child: Column(
          ...
          children: [
            Assets.lottie.somethingWentWrong.lottie(),
            HiddenButton(
              child: Icon(
                Icons.code,
                color: context.colorTheme.onBackground.withOpacity(.1),
              ),
            ),
            if (kDebugMode)
              ExpansionTile(
  ...
  static Widget reload(error, Function()? onReload, [stackTrace]) =>
      ErrorView(error: error, onReload: onReload, stackTrace: stackTrace);
